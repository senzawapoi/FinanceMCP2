    ...
    try {
      ...
      try {
        ...
        if (requestedIndicators.length > 0 && ['cn', 'us', 'hk', 'fund'].includes(marketType)) {
          ...
          const closes = stockData.map((d: Record<string, any>) => parseFloat(d.close)).reverse(); // 按时间正序
          const highs = stockData.map((d: Record<string, any>) => parseFloat(d.high)).reverse();
          const lows = stockData.map((d: Record<string, any>) => parseFloat(d.low)).reverse();
