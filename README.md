...
欢迎使用 **FinanceMCP 财经数据服务器**！本项目提供一个模型上下文协议 (MCP) 服务器，使语言模型（如 Cline）能够访问通过 **Tushare API** 获取的实时财经新闻、股票数据、指数数据和宏观经济数据。这使得基于最新的市场信息进行富有洞察力的分析和预测成为可能。

本指南将引导您完成此服务器的设置、配置和使用，即使您是 MCP 或 Node.js 开发的新手也能轻松上手。

## 🌟 功能特性

*   **全面的金融数据访问**：通过 Tushare API 获取股票、指数、财经新闻和宏观经济数据。
*   **股票数据访问**：获取指定股票代码的历史行情数据。
*   **指数数据访问**：获取如上证指数、深证成指等主要市场指数的历史行情数据。
*   **财经新闻检索**：从多种来源（通过 Tushare API）收集最新的财经新闻。
*   **宏观经济数据**：获取Shibor利率、LPR利率、GDP、CPI、PPI等宏观经济指标数据。
...
您也可以使用 `macro_econ` 工具查询宏观经济数据，例如国内生产总值 (GDP)、消费者价格指数 (CPI) 等。

*"查询最近几年的GDP数据，并分析其变化趋势"*

Cline 将识别需要宏观经济数据，并使用 `macro_econ` 工具。

*   **工具调用：** Cline 将向您的服务器发送请求以使用 `macro_econ` 工具。
    ```json
    {
      "indicator": "gdp",  // 可选：shibor, lpr, gdp, cpi, ppi
      "start_date": "20180101",
      "end_date": "20231231"
    }
    ```

*   **服务器响应：** 您的服务器将通过 Tushare API 获取GDP数据并将其发送回 Cline，然后Cline会分析这些数据，识别经济增长趋势和影响因素。
...
### 6. 扩展宏观经济数据分析
*   **描述**：进一步扩展宏观经济数据工具，添加更多的经济指标和分析功能，如失业率、利率、货币供应量等。
*   **益处**：为用户提供更全面的宏观经济视角，帮助理解市场大环境。
*   **实现思路**：
    *   扩充现有的 `macro_econ` 工具，支持更多的经济指标。
    *   添加指标之间的关联分析功能，如CPI与GDP增长的关系分析。
    *   整合国际市场数据，提供跨市场比较分析。
