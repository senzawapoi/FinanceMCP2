...
export interface UsBalanceItem {
  ts_code: string;
  end_date: string;
  ind_type: string;
  name: string;
...
// 格式化美股资产负债表数据
export function formatUsBalanceData(data: UsBalanceItem[], ts_code: string, dataType: string) {
...
// 按报告期分组数据
function groupByPeriod(data: UsBalanceItem[]): { [period: string]: UsBalanceItem[] } {
  return data.reduce((groups, item) => {
    ...
    return groups;
  }, {} as { [period: string]: UsBalanceItem[] });
}
...
// 分类资产负债表项目
function categorizeBalanceItems(items: UsBalanceItem[]) {
  const assets: UsBalanceItem[] = [];
  const liabilities: UsBalanceItem[] = [];
  const equity: UsBalanceItem[] = [];
...
// 获取资产负债表科目分类
function getBalanceCategory(indName: string): string {
  const assetKeywords = [
  ...
  for (const keyword of assetKeywords) {
    if (indName.includes(keyword)) return 'asset';
  }
  
  for (const keyword of liabilityKeywords) {
    if (indName.includes(keyword)) return 'liability';
  }
  
  for (const keyword of equityKeywords) {
    if (indName.includes(keyword)) return 'equity';
  }
...
// 生成资产负债表关键指标分析
function generateBalanceKeyMetrics(items: UsBalanceItem[], period: string): string {
